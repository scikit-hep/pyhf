...
def final_objective(pars, objective, tv, fixed_vals, data, model):
...
jitted_objective_and_grad = jax.jit(
    jax.value_and_grad(final_objective), static_argnums=(1, 2, 3, 4, 5)
)
    ...
    def setup_minimize(
        ...
        def func(pars):
            a, b = jitted_objective_and_grad(
                pars, objective, tv, fixed_values_tensor, data, pdf
            )
