...
def get_interval(mu, sigma, level, teststat):
    muhat_over_sigma = np.asarray(
        [-1000, -900] + np.linspace(-50, 50, 5000).tolist() + [900, 1000]
    )
...
def plot_neyman_construction(ax, min_mu, max_mu, hypos, scans, delta=True):
    if not delta:
        ...
        ax.hlines(
            hypos, hypos + scans[:, 1], hypos + scans[:, 2], colors='k', alpha=0.2
        )
    else:
...
def plot_cuts(ax, hypos_over_sigma, atcut, sigma, teststat):
    ...
    for n, c in zip([-2, -1, 0, 1, 2], colors):
        ax.plot(
            hypos_over_sigma,
            teststat(hypos_over_sigma * sigma, n * sigma, sigma=sigma),
            c=c,
        )
    ax.set_xlabel('μ/σ')
    ax.set_ylabel('test statistic')
...
def plot_pvalue(ax, hypos_over_sigma, scans, sigma, teststat, cuts):
    ...
    for c, n in zip(colors, [-2, -1, 0, 1, 2]):
        ax.plot(
            hypos_over_sigma,
            [
                np.interp(teststat(h, n, sigma=1), s[:-1, 0][::-1], s[:-1, -1][::-1])
                for s, h in zip(scans, hypos_over_sigma)
            ],
            c=c,
        )

    cuts = [
        np.interp(c, s[:-1, 0][::-1], s[:-1, -1][::-1])
        for c, s, h in zip(cuts, scans, hypos_over_sigma)
    ]
