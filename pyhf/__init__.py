...
def _poisson_impl(n, lam):
    ...
    # print('lam, sqrtlam: {}, {}'.format(lam, np.sqrt(lam)))
...
def _hfinterp_code0(at_minus_one, at_zero, at_plus_one):
    ...
    def func(alpha):
        if alpha > 0:
            return (at_plus_one - at_zero) * alpha
        if alpha <= 0:
            return (at_zero - at_minus_one) * alpha
    return func
...
def _hfinterp_code1(at_minus_one, at_zero, at_plus_one):
    ...
    def func(alpha):
        if alpha > 0:
            return pow(at_plus_one / at_zero, alpha)
        if alpha <= 0:
            return pow(at_minus_one / at_zero, -alpha)
    return func
...
def _multiply_arrays_or_scalars(*args):
    '''
    Calling this function with arrays (a and b) and/or scalers (s1,s2) it will broadcast
    as usual. The result is a array with the row-wise products
    r1 = | a1 | * s1 * | b1 | * s2
    r2 = | a2 | * s1 * | b2 | * s2
    r3 = | a3 | * s1 * | b3 | * s2
    '''
...
class hfpdf(object):
            ...
            for mod_def in sample_def['mods']:
                ...
                if mod_def['type'] == 'shapesys':
                    ...
                    mod = shapesys_constraint(sample_def['data'],
                                              mod_def['data'])
                    self.config.add_mod(mod_def['name'],
                                        npars=len(sample_def['data']), mod=mod)
                ...
                if mod_def['type'] == 'normsys':
                    mod = normsys_constraint(sample_def['data'],
                                             mod_def['data'])
                    self.config.add_mod(mod_def['name'],
                                        npars=len(sample_def['data']), mod=mod)
                    # it's a constraint, so this implies more data
                ...
                if mod_def['type'] == 'histosys':
                    mod = histosys_constraint(sample_def['data'],
                                              mod_def['data'])
                    self.config.add_mod(mod_def['name'],
                                        npars=len(sample_def['data']), mod=mod)
                    # it's a constraint, so this implies more data
    ...
    def _multiplicative_factors(self, sample, pars):
        ...
        mods = [m['name'] for m in self.samples[sample]['mods']
    ...
    def _normsysfactor(self, sample, pars):
        ...
        for m in mods:
            ...
            assert len(val) == 1
            interp = _hfinterp_code1(mod.at_minus_one,
                                     mod.at_zero, mod.at_plus_one)
    ...
    def constraint_pdf(self, auxdata, pars):
        ...
        for cname in self.config.all_mods(only_constraints=True):
            # print('ADDING constraint pdf term for {}'.format(cname))
            mod, modslice = self.config.mod(
                cname), self.config.par_slice(cname)
...
def generate_asimov_data(asimov_mu, data, pdf, init_pars, par_bounds):
    bestfit_nuisance_asimov = constrained_bestfit(asimov_mu, data, pdf,
...
def qmu(mu, data, pdf, init_pars, par_bounds):
    ...
    if muhatbhat[0] > mu:
        return 0.0
    if -1e-6 < qmu < 0:
        print('WARNING: qmu negative: {}'.format(qmu))
        return 0.0
    return qmu
